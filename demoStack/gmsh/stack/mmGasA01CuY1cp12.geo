...
For m1 In { 0:numA01cpUy }
 For n1 In { 0:numA01cpUx }
    ...
    llA01cp12Ib5~{m1}~{n1} = newll; Line Loop ( llA01cp12Ib5~{m1}~{n1} ) = { lA01cp1Bb1~{m1}~{n1}, lA01cp1Bb2~{m1}~{n1}, lA01cp1Bb3~{m1}~{n1}, lA01cp1Bb4~{m1}~{n1} };
    llA01cp12Ib5A[] += llA01cp12Ib5~{m1}~{n1};
    psA01cp12Ib5A~{m1}~{n1} = news; Plane Surface ( psA01cp12Ib5A~{m1}~{n1} ) = { llA01cp12Ib5~{m1}~{n1} };
    surfA01cp1Bb5A[] += psA01cp12Ib5A~{m1}~{n1};
    // Transfinite Surface { psA01cp12Ib5A~{m1}~{n1} };
    // Recombine Surface { psA01cp12Ib5A~{m1}~{n1} };

    //----------------------------------------------------------
    //// Top boundary

    llA01cp12Ib6~{m1}~{n1} = newll; Line Loop ( llA01cp12Ib6~{m1}~{n1} ) = { lA01cp2Tb1~{m1}~{n1}, lA01cp2Tb2~{m1}~{n1}, lA01cp2Tb3~{m1}~{n1}, lA01cp2Tb4~{m1}~{n1} };
    psA01cp12Ib6A~{m1}~{n1} = news; Plane Surface ( psA01cp12Ib6A~{m1}~{n1} ) = { llA01cp12Ib6~{m1}~{n1} };
    surfA01cp2Tb6A[] += psA01cp12Ib6A~{m1}~{n1};
    // Transfinite Surface { psA01cp12Ib6A~{m1}~{n1} };
    // Recombine Surface { psA01cp12Ib6A~{m1}~{n1} };

    //----------------------------------------------------------
    //// Copper strip and surface loop - anode surface loop

    // slA01cpSb1~{m1}~{n1} = newsl; Surface Loop ( slA01cpSb1~{m1}~{n1} ) = { psA01cp12Ib1~{m1}~{n1}, psA01cp12Ib2~{m1}~{n1}, psA01cp12Ib3~{m1}~{n1}, psA01cp12Ib4~{m1}~{n1}, psA01cp12Ib5A~{m1}~{n1}, psA01cp12Ib6A~{m1}~{n1} };
    // slA01cpSb1[] += slA01cpSb1~{m1}~{n1};
...
volA01cpV1 = newv; Volume ( volA01cpV1 ) = slA01cpSb1A[];
